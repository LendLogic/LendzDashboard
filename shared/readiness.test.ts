...
test('exposes every registry module in tab order', () => {
  expect(MODULES.map((m) => m.key)).toEqual(['pe', 'vt', 'uw', 'lexi', 'broker', 'docmagic', 'bank', 'id', 'pl', 'paystub', 'appraisal', 'credit', 'tax1040', 'taxbiz', 'k1', 'form1099', 'w2', 'voe', 'title', 'insurance'])
})

test('flags exactly the assumed modules', () => {
  const assumed = MODULES.filter((m) => m.assumed).map((m) => m.key)
  expect(assumed.sort()).toEqual(['appraisal', 'bank', 'broker', 'credit', 'docmagic', 'form1099', 'id', 'insurance', 'k1', 'paystub', 'pl', 'tax1040', 'taxbiz', 'title', 'voe', 'vt', 'w2'])
})
